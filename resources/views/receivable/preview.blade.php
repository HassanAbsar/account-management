...
<body>
    <div class="container">
        ...
        <table class="company-details">
            <tr>
                <td>
                    <b>Sale to:</b> {{ $receivable->customer }}
                </td>
            </tr>
            <tr>
                <td><b>Area:</b> {{ $receivable->city->name }} </td>
            </tr>
            <tr>
                <td><b>Phone:</b> {{ $receivable->contact_no }}</td>
            </tr>
            <tr>
                <td><b>Saleman:</b> {{ $receivable->saleman }}</td>
            </tr>
        ...
        <table class="company-details" style="margin-left: 500px;margin-top: -200px;">
            <tr>
                <td>
                    <b>Invoice #</b> {{ $receivable->invoice_no }}
                </td>
            </tr>
            <tr>
        ...
        <table class="item-table">
            ...
            <tbody>
                @foreach ($receivable_details as $receivable_detail)
                <tr>
                    <td>{{ $receivable_detail->product_name}}</td>
                    <td>{{ $receivable_detail->price }}</td>
                    <td>{{ $receivable_detail->qty }}</td>
                    <td>{{ $receivable_detail->total }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
...
function numberToWordss($num) {
    ...
    return ucfirst(trim($string)); // Adds "only" once at the end
            ...
            $convertedNumber = numberToWords($receivable->total);

        @endphp 

        <table class="no-border-table" style="margin-left: 500px;">
            <tr>
                <td style="text-align: end;"><b>Total Amount :</b>  {{ $receivable->total }}</td>
            </tr>
            <tr>
                <td><b>Remaining :</b> {{ $receivable->remaining }}</td>
            </tr>
            <tr>
                <td><b>Paid :</b> {{ (int) $receivable->total - (int) $receivable->remaining }}</td>
            </tr>
        </table>
        <table>
            <tr>
