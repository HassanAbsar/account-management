...
@section('content')
<div class="card">
    <form method="post" action="{{ route('payable.update',$payable->id) }}" enctype="multipart/form-data">
        @csrf
        <div class="card-body">
            <div class="row">
                <div class="col-lg-3 col-sm-4 col-12">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="name" name="invoice_no"
                            value="{{ $payable->invoice_no }}" placeholder="Enter Invoice No">
                    </div>
                </div>
                <div class="col-lg-3 col-sm-4 col-12">
                    <div class="mb-3">
                        ...
                        value="{{ \Carbon\Carbon::parse($payable->date)->format('Y-m-d') }}">
                    </div>
                </div>
                <div class="col-lg-3 col-sm-4 col-12">
                    <div class="mb-3">
                        ...
                        <select name="created_by" class="form-select">
                            ...
                            <option value="Obaid" @selected($payable->created_by == "Obaid")>Obaid</option>
                            <option value="Uzair" @selected($payable->created_by == "Uzair")>Uzair</option>
                        </select>
                    </div>
                </div>
                <div class="col-lg-3 col-sm-4 col-12">
                ...
                <div class="col-lg-3 col-sm-4 col-12">
                    <div class="mb-3">
                        ...
                        <select name="city_id" class="form-select">
                            ...
                            <option value="{{ $city->id }}" @selected($payable->city_id == $city->id )>{{ $city->name }}
                            </option>
                            @endforeach
                        </select>
                    </div>
                ...
                <table id="afterChangeUploadedFilesTable" class="table table-condensed table-bordered table-striped">
                    ...
                    <tbody>
                        @if (isset($payable->attachments))
                            @php
                                $after_attachmentFiles = explode(',', $payable->attachments);
                                $uniqueAttachments = [];
                            @endphp
                            @foreach ($after_attachmentFiles as $attachment)
                                @if (!in_array(substr($attachment, 14), $uniqueAttachments))
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex">
                                                <a target="_blank" href="{{ asset('/payable-after_attachments/') }}/{{ $attachment }}" class="btn mx-2 btn-outline-primary">
                                                    <i class="fas fa-file"></i>
                                                </a>
                                                <button type="button" class="btn btn-danger mx-2 delete-attachment" data-file="{{ $attachment }}">
                                                    X
                                                </button>
                                            </div>
                                        </td>
                ...
                <div class="table-responsive">
                    <table class="display" id="products">
                        ...
                        <tbody>
                            @foreach($payable_details as $key => $product )
                            <tr>
                                <td>{{ $loop->iteration}}</td>
                                <td><input type="text" class="form-control product" name="product[{{$key}}][product_name]" value="{{  $product['product_name'] }}"></td>
                                <td><input type="number" class="form-control unit-price"  name="product[{{$key}}][price]" value="{{ (float) $product['price'] }}"></td>
                                <td><input type="number" class="form-control quantity"  name="product[{{$key}}][qty]" value="{{ (float) $product['qty'] }}"></td>
...
let fileUploadInput;
        ...
        document.querySelectorAll('.nameFile').forEach(function(input) {
            ...
            if (fileType === 'before') {
                url = `${baseUrl}/payable-before_attachments/${fileName}`;
            } else if (fileType === 'after') {
                url = `${baseUrl}/payable-after_attachments/${fileName}`;
            }
