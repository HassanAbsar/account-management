<?php

namespace App\Http\Controllers;

use App\Models\City;
use App\Models\Payment;
use App\Models\Receivable;
use App\Models\ReceivableDetail;
use Carbon\Carbon;
use Illuminate\Http\Request;
use Illuminate\Validation\Rule;
use Barryvdh\DomPDF\Facade\Pdf as FacadePdf;
use Yajra\DataTables\Facades\DataTables;

class ReceivableController extends Controller

{
    //
    public function Data(Request $request)
    {
        if ($request->ajax()) {

            $query = Receivable::select(
                'receivables.id',
                'receivables.invoice_no',
                'receivables.created_by',
                'receivables.city_id',
                'receivables.date',
                'receivables.payment_status',
                'receivables.customer',
                'receivables.saleman',
                'receivables.total',
                'receivables.remaining',

            )->with('city');


            $datatable = DataTables::eloquent($query)
                ->addIndexColumn()
                
                ->addColumn('action', function($row){
                    $receivable_detail = ReceivableDetail::where('receivable_id', $row->id)->get();
                    $payment = Payment::where('receivable_id',$row->id)->first();
                    $remaining = $row->remaining; 
                    if (isset($payment)) {
                        $remaining = isset($payment) ? $payment->remaining : 0; 
                    }
                    $payment_id = isset($payment) ? $payment->id : 0;
                    $account_no = isset($payment) ? $payment->account_no : 0; 
                    $paying_amount = isset($payment) ? $payment->paying_amount : 0; 
                    $payment_method = isset($payment) ? $payment->payment_method : 0; 
                    $payment_term = isset($payment) ? $payment->payment_term : 0; 
                    $payment_id = isset($payment) ? $payment->id : 0;
                    $btn = '<div class="d-flex"><a class="btn btn-info btn-sm mx-2" data-tooltip="Edit" href="' .
                    route('receivable.edit', $row->id) .
                    '"><i class="fas fa-pencil"></i></a>
                    <a class="btn btn-success btn-sm mx-2" data-tooltip="View" href="' .
                    route('receivable.view', $row->id) .
                    '"><i class="fas fa-eye"></i></a>
                     <a class="btn btn-danger btn-sm mx-2" data-tooltip="Delete" href="' .
                    route('receivable.destroy', $row->id) .
                    '"><i class="fas fa-trash"></i></a><a type="button" class="btn btn-primary btn-sm" id="addPaymentModal" data-bs-toggle="modal" data-bs-target=".bd-example-modal-lg" data-tooltip="Add Payment" data-id="' . $row->id . '" data-total-amount="' . $row->total . '" data-remaining-balance="' . $remaining . '" data-payment_id="'.$payment_id.'" data-account_no="'.$account_no.'" data-payment_method="'.$payment_method.'" data-paying_amount="'.$paying_amount.'" data-payment_term="'.$payment_term.'" title="Add Payment"><i class="fas fa-money-bill"></i></a>
                    <a class="btn btn-warning btn-sm mx-2" data-tooltip="Payment History" href="' .
                    route('payments.history', $row->id) .
                    '"><i class="fas fa-file-invoice"></i></a>
                     <a class="btn btn-danger btn-sm mx-1" data-tooltip="PDF" href="' . route('receivable.preview', $row->id) .'" target="_blank"><i class="fas fa-file-pdf"></i></a>
                    </div>';
                    return $btn;
                })
                ->rawColumns(['action']);

                if($request->search['value'] == null ){

                    $datatable = $datatable->filter(function ($query) use ($request) {
                    if ($request->has('invoice_no') && !is_null($request->invoice_no)) {
                        $query->where('invoice_no', 'like', "%{$request->invoice_no}%");
                    }
                    if ($request->has('created_by') && !is_null($request->created_by)) {
                        $query->where('created_by', 'like', "%{$request->created_by}%");
                    }
                    if ($request->has('date') && !is_null($request->date)) {
                        $query->where('date', 'like', "%{$request->date}%");
                    }
                    if ($request->has('city') && !is_null($request->city)) {
                        $query->whereHas('city', function ($q) use ($request) {
                            $q->where('cities.name', 'like', "%{$request->city}%");
                        });
                    }
                    if ($request->has('payment_status') && !is_null($request->payment_status)) {
                        $query->where('payment_status', 'like', "%{$request->payment_status}%");
                    }
                    if ($request->has('customer') && !is_null($request->customer)) {
                        $query->where('customer', 'like', "%{$request->customer}%");
                    }


                });
            }

               return $datatable->make(true);
        }

    }


    public function index(){
      return view('receivable.index');
  }
    public function create()
    {
        $cities = City::select('id','name')->get();
      return view('receivable.create',compact('cities'));
    }

    public function store(Request $request)
    {
         $total = 0;
        foreach ($request->product as $key => $product) {
            $total = $total + (int)$product['total'];
        }

      $receivable = new Receivable();
      $receivable->invoice_no = $request->invoice_no;
      $receivable->date = Carbon::parse($request->date)->format('d-m-Y');
      $receivable->created_by = $request->created_by;
      $receivable->acc_no = $request->acc_no;
      $receivable->customer = $request->customer;
      $receivable->saleman = $request->saleman;
      $receivable->address = $request->address;
      $receivable->city_id = $request->city_id;
      $receivable->contact_no = $request->contact_no;
      $receivable->total = $total;
      $receivable->remaining = $total;
      $receivable->term = $request->term;
      $filenames2 = [];
      foreach ($request->file() as $key => $file) {
          if (str_contains($key, 'after_attachment_')) {
              $filename2 = date('YmdHis') . $file->getClientOriginalName();
              $file->move('receivable-after_attachments', $filename2);
              $filenames2[] = $filename2;
          }
      }
      if (count($filenames2) > 0) {
          $receivable->attachments = implode(',', $filenames2);
      } else {
          $receivable->attachments = null;
      }
      $receivable->save();

      foreach ($request->product as $key => $product) {
        $receivable_detail = new ReceivableDetail();
        $receivable_detail->receivable_id = $receivable->id;
        $receivable_detail->product_name = $product['product_name'];
        $receivable_detail->price = $product['price'];
        $receivable_detail->qty = $product['qty'];
        $receivable_detail->total = $product['total'];
        $receivable_detail->save();
      }
      return redirect()->route('receivable.index')->with('success', 'Receivable Created Successfully.');
    }

    public function edit($id)
    {
     
        $receivable = Receivable::find($id);
        $receivable_details =  ReceivableDetail::where('receivable_id',$id)->get();
        $cities = City::select('id','name')->get();
        return view('receivable.edit',compact('cities','receivable','receivable_details'));
    }

    public function update(Request $request, $id)
    {

         $total = 0;
        foreach ($request->product as $key => $product) {
            $total = $total + (int)$product['total'];
        }

        $receivable = Receivable::find($id);
        $receivable->invoice_no = $request->invoice_no;
        $receivable->date = Carbon::parse($request->date)->format('d-m-Y');
        $receivable->created_by = $request->created_by;
        $receivable->acc_no = $request->acc_no;
        $receivable->customer = $request->customer;
        $receivable->address = $request->address;
        $receivable->saleman = $request->saleman;
        $receivable->city_id = $request->city_id;
        $receivable->contact_no = $request->contact_no;
        $receivable->term = $request->term;
        $receivable->total = $total;
        $receivable->remaining = $total;
        $filenames2 = [];
         if ($request->hasFile('attachments')) {
        foreach ($request->file() as $key => $file) {
            if (str_contains($key, 'after_attachment_')) {
                $filename2 = date('YmdHis') . $file->getClientOriginalName();
                $file->move('receivable-after_attachments', $filename2);
                $filenames2[] = $filename2;
            }
        }
            $receivable->attachments = implode(',', $filenames2) . ',' . $request->prev_attachments;
        } else {
            $receivable->attachments = $request->prev_attachments;
        }
        $receivable->save();
        ReceivableDetail::where('receivable_id',$id)->delete();
        foreach ($request->product as $key => $product) {
          $receivable_detail = new ReceivableDetail();
          $receivable_detail->receivable_id = $receivable->id;
          $receivable_detail->product_name = $product['product_name'];
          $receivable_detail->price = $product['price'];
          $receivable_detail->qty = $product['qty'];
          $receivable_detail->total = $product['total'];
          $receivable_detail->save();
        }
      return redirect()->route('receivable.index')->with('success', 'Receivable Updated Successfully.');
    }

    public function view($id)
    {
        $receivable = Receivable::find($id);
        $receivable_details =  ReceivableDetail::where('receivable_id',$id)->get();
        $cities = City::select('id','name')->get();
        return view('receivable.view',compact('cities','receivable','receivable_details'));
    }

    public function destroy($id)
    {
    
      $receivable = Receivable::find($id);
      $payable_detail = ReceivableDetail::where('receivable_id',$id)->delete();
      $receivable->delete();
      return redirect()->route('receivable.index')->with('success', 'Receivable Deleted Successfully.');
    }


    public function preview(Request $request, $id){
       
        $receivable = Receivable::with('city')->find($id);
        $receivable_details =  ReceivableDetail::where('receivable_id',$id)->get();
        $cities = City::select('id','name')->get();
        $pdf = FacadePdf::loadView('receivable.preview', compact('receivable', 'receivable_details', 'cities'))
        ->setPaper('a4', 'portrait') 
        ->setOptions([
            'defaultFont' => 'DejaVu Sans', 
            'isHtml5ParserEnabled' => true,
            'isPhpEnabled' => true,
        ]);
        return $pdf->stream('invoice.preview');
    }

}
